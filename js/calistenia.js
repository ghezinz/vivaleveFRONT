document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(treinos => {
        const treinosCalistenia = treinos.filter(treino => treino.tipo === "calistenia");
        const container = document.getElementById("calisteniaContainer");

        if (treinosCalistenia.length === 0) {
            container.innerHTML = "<p>Nenhum treino encontrado para calistenia.</p>";
            return;
        }

        treinosCalistenia.forEach(treino => {
