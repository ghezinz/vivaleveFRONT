document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(treinos => {
        const treinosMusculacao = treinos.filter(treino => treino.tipo === "musculação");
        const container = document.getElementById("musculacaoContainer");

        if (treinosMusculacao.length === 0) {
            container.innerHTML = "<p>Nenhum treino encontrado para musculação.</p>";
            return;
        }

        treinosMusculacao.forEach(treino => {
